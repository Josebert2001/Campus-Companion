// supabase/functions/study-rooms/index.ts - Free WebRTC Study Rooms
...
/*
Frontend WebRTC Integration Example:
...
class StudyRoomManager {
  ...
  async createRoom(roomName: string, subject: string) {
    const response = await fetch('/functions/v1/study-rooms', {
      method: 'POST',
      headers: {
  ...
  private async setupWebRTC(iceServers: RTCIceServer[]) {
    ...
    try {
      this.localStream = await navigator.mediaDevices.getUserMedia({
        audio: true,
        video: false // Audio-only for study rooms
      });
  ...
  async leaveRoom() {
    ...
    if (this.roomId) {
      await fetch('/functions/v1/study-rooms', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${session.access_token}`,
          'Content-Type': 'application/json'
...
```tsx
import { StudyRoomManager } from '@/utils/studyRooms';

export default function StudyRoom() {
  const [roomManager] = useState(() => new StudyRoomManager());
  const [messages, setMessages] = useState([]);
  const [newMessage, setNewMessage] = useState('');

  const createRoom = async () => {
    try {
      const result = await roomManager.createRoom('Math Study', 'Mathematics');
      console.log('Room created:', result.joinUrl);
    } catch (error) {
  ...
  const sendMessage = async () => {
    ...
    try {
      const result = await roomManager.sendMessage(newMessage);
      if (result.success) {
...
- GROQ_API_KEY (for AI responses in chat)
- FRONTEND_URL (for room join URLs)
- Standard Supabase variables

This gives you:
✅ Free WebRTC peer-to-peer audio
✅ Real-time text chat with AI integration  
✅ Study room management
✅ No external costs
✅ Scales well for small groups
*/